	...
	for Te in lst_Te:
		...
		for filename in flnms:
			...
			with open(filename, 'r') as file:
				...
				srch = re.search('V0.*1.0.*\n', txt)
				print(srch)
				if srch is not None:
					V0 = srch.group()
					print(V0.split()[-1])
# 					print(srch.group().spilt()[-1])
	...
	# 				common = common + srch.group()
		...
		# 		txt = re.sub(x, upx, txt)
			...
			#update 
				...
				txt = txt.replace(V0.split()[-1], str(Te))
	
			with open(f'../Te/test/{molecule}_{Te}_e-0.inp', 'w') as file:
				file.write(txt)
		
			with open(f'../Te/test/{molecule}_{Te}_e-0.inp', 'r') as file:
				txt = str(file.read())
				deltam = 1+ 10**(-4)
			...
			with open(f'../Te/test/{molecule}_{Te}_e-4.inp', 'w') as file:
				file.write(txt)
